...
4. ~~Ingest performance~~ — **implemented 2026-06-11** (plan_03 P1 embed cache + P2 batched prewarm + P3 chunk concurrency). Measured on a synthetic 59-chunk workload with recurring entity names (counting fake-client calls, old code via a HEAD worktree): provider embed calls **594 → 62 (9.6× fewer HTTPS round-trips)**, texts embedded 594 → 244 (2.4× via dedup+cache). This validates the mechanism behind the old "~900 serial round-trips ≈ 35 min" bottleneck; **real-corpus Gemini wall-clock has not been re-measured** — record it on the next paid full ingest. Concurrency knob: `IngestConfig.concurrency` (env `MEMBOX_INGEST_CONCURRENCY`, default 1 = serial, bit-identical to old behavior; DB writes stay serialized in chunk order).
5. ~~Residual recall misses (q08, q19)~~ — **resolved 2026-06-11**: both were 2000-token budget/ranking tradeoffs; at the post-resnapshot eval budget of 4000 the full Gemini run is 26/26 (see Notes baseline entry). No retrieval-algorithm change was needed (owner decision: do not pursue per-question algorithm work for budget-bound misses).
