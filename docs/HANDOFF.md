...
- **C5 acceptance metrics (heuristic-v1 gate vs C1 fixtures, recorded 2026-06-11, harness `tests/test_lifecycle_acceptance.py`)**: triage precision 1.00, triage recall 0.83 (c4 "Fact:"-prefix not recognized as durable signal), type accuracy 0.33 (c1/c5 keyword tie-break fires the wrong type; c7 `更正:` not a durable-change signal), duplicate rate 0.00. The 4 disagreements are pinned as strict xfails in the harness — they are the keyword-table tuning backlog for the gate, NOT fixture bugs; do not edit `expectations.yaml` to make them pass.
