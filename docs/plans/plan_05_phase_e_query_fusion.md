...
- Ranking score: `relevance × stored_score × recency_factor`, where `stored_score` combines `importance_score` and `confidence_score` (spec_02 §query-fusion names all three components: query relevance, stored importance/confidence, recency). Proposed first cut: `stored_score = importance_score × confidence_score` (both already 0–1; a high-importance low-confidence unit should rank below an equally important confirmed one — confidence is exactly what Phase D score evolution maintains). Relevance from FTS rank (BM25, normalized like the chunk pool); recency a step-down factor on age since `COALESCE(last_recalled_at, updated_at)` (spec names both; if E4 resolves to "defer", `last_recalled_at` is always NULL and this degrades cleanly to `updated_at`). Weights as named constants (`MEMORY_RANK_*`) with docstrings marking them owner-calibration targets.
